...
interface SolucionPostPageProps {
...
export async function generateMetadata({
  params,
}: SolucionPostPageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPost(slug);

  if (!post) {
    return { title: "Solución no encontrada" };
  }

  return {
    title: post.title,
    description:
      post.excerpt ?? `Conoce "${post.title}" - Soluciones GenioX Commerce`,
    openGraph: {
      title: post.title,
...
export default async function SolucionPostPage({
  params,
}: SolucionPostPageProps) {
  const { slug } = await params;
  const post = await getPost(slug);
